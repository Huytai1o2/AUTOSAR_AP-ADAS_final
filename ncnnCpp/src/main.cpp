...
static int detect_yolo11(const cv::Mat& bgr, std::vector<Object>& objects, std::vector<float>& inferenceTimes)
{
    ncnn::Net yolo11;

    yolo11.opt.use_vulkan_compute = true;
    yolo11.opt.num_threads = 4;
    // yolo11.opt.use_bf16_storage = true;

    // https://github.com/nihui/ncnn-android-yolo11/tree/master/app/src/main/assets
    yolo11.load_param("../best_ncnn_model/best.ncnn.param");
    yolo11.load_model("../best_ncnn_model/best.ncnn.bin");
    ...
    double inference_time = (t2 - t1) / cv::getTickFrequency() * 1000;
    printf("Inference time: %.2f ms\n", inference_time);

    inferenceTimes.push_back(inference_time);
...
static void draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects)
{
    static const char* class_names[] = {
        "green-lights", "red-lights", "traffic-lights", "turn-left", "turn-right", "yellow-lights"
    };
    ...
    printf("Saved result image as output.jpg\n");
}
...
int main(int argc, char** argv)
{
    ...
    // Parse flags: -data=xxx -numTest=xxx
    for (int i = 1; i < argc; i++)
    {
        std::string arg = argv[i];
        if (arg.rfind("-data=", 0) == 0)
        {
            imagePath = arg.substr(6); // delete "-data="
        }
        else if (arg.rfind("-numTest=", 0) == 0)
        {
            numTest = std::stoi(arg.substr(9)); // delete "-numTest="
        }
        else
        {
    ...
    if (imagePath.empty())
    {
        fprintf(stderr, "Usage: %s -data=imagepath -numTest=N\n", argv[0]);
        return -1;
    }
    ...
    {
        detect_yolo11(m, objects, inferenceTimes);
        draw_objects(m, objects);
    }

    float meanInferenceTime = 0.0f;
    for (float t : inferenceTimes)
        meanInferenceTime += t;

    meanInferenceTime /= inferenceTimes.size();
    printf("Mean Inference Time over %zu runs: %.2f ms\n", inferenceTimes.size(), meanInferenceTime);
