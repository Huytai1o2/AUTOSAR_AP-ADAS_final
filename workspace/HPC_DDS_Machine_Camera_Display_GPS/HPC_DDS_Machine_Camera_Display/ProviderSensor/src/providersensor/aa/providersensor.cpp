...
// Handling UART connection and GPS reading
...
void ProviderSensor::Run()
{
    ...
    m_workers.Async([this] {
        ...
        try {
            nodes = parseGeoJSON(GEOJSON_FILE_NAME);
        } catch (const std::exception& e) {
            m_logger.LogError() << "[GPS] Failed to load GeoJSON: " << e.what();
            return;
        }
        if (nodes.empty()) {
            m_logger.LogError() << "[GPS] GeoJSON contains no traffic nodes.";
            return;
        }
        KDTree tree(std::move(nodes));
        m_logger.LogInfo() << "[GPS] KD-tree ready. Waiting for UART...";

        // Open UART – retry while the app is running
        int gpsFd = -1;
        ...
        if (gpsFd < 0) return; // app stopped before UART became available

        m_logger.LogInfo() << "[GPS] UART open. Waiting for fix...";
        ...
        while (m_running) {
            ...
            if (!readNextGPSFix(gpsFd, gpsCoord)) {
                ...
                while (m_running && gpsFd < 0) {
                    ...
                    gpsFd = openGPSUART();
                    if (gpsFd < 0)
                        m_logger.LogWarn() << "[GPS] Device unavailable, retrying in "
                                           << static_cast<int32_t>(GPS_RETRY_DELAY) << " s...";
                }
                if (gpsFd >= 0)
                    m_logger.LogInfo() << "[GPS] Reconnected. Waiting for fix...";
                continue;
            }

            // KNN query: find nearest traffic node
            try {
                ...
                if (dist <= static_cast<double>(RANGE_SIZE)) {
                    m_logger.LogWarn() << "[GPS] ALERT: Within " << static_cast<int32_t>(RANGE_SIZE)
                                       << "m of traffic node id=" << static_cast<int64_t>(nn.id)
                                       << " (dist=" << dist << "m)"
                                       << " node_lat=" << nn.coords.latitude
                                       << " node_lon=" << nn.coords.longitude;
                }
            } catch (const std::exception& e) {
                m_logger.LogError() << "[GPS] KD-tree query error: " << e.what();
            }
        }
